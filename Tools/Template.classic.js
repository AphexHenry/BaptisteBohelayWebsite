 ...
 *
 * Classic IIFE for experiment pages and any HTML that loads only synchronous scripts.
 * Main site uses ./registerTemplateGlobals.mjs + ./Template.mjs instead.
 * For new ES modules, import from ./Template.mjs.
 */
(function (global) {
	'use strict';

	/** Matches legacy PI2 from index.html (-Math.PI * 1.99); kept local so this file is self-contained. */
	var PI2 = -Math.PI * 1.99;
	...
	function programTriangle(context) {
		programFill(context);
	...
	function programTriangleStroke(context) {
	...
	function programDoNothing() {}

	function programPauseStroke(context) {
		programStroke(context);
	...
	function programStrokeThick(context) {
	...
	function programStrokeLimited(context) {
		context.lineWidth = 0.04;
		context.beginPath();
	...
	function programFillWater(context) {
		context.beginPath();
	...
	global.programTriangle = programTriangle;
	global.programTriangleStroke = programTriangleStroke;
	global.programDoNothing = programDoNothing;
	global.programPauseStroke = programPauseStroke;
	global.programPause = programPause;
	global.programStrokeThick = programStrokeThick;
	global.programStroke = programStroke;
	...
	global.GetRectangleFaded = GetRectangleFaded;
	global.GetRectangleBack = GetRectangleBack;
	global.GetNoise = GetNoise;
	global.SetTextInCanvas = SetTextInCanvas;
	global.GetTextInBox = GetTextInBox;
	global.GetImageInRectangle = GetImageInRectangle;
	global.GetAvatarCircle = GetAvatarCircle;
