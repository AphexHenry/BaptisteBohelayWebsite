...
function ParticleSound(aPositionHome, aPositionCenter, volume, aTargetObject) 
{
	this.name = aTargetObject.name;
	this.url = aTargetObject.url;

	var particle = new THREE.Particle( new THREE.ParticleCanvasMaterial( { color: PickColor(), program: programTriangleStroke, transparent:true } ) );
	...
	this.mPositionCenter = aPositionCenter.clone();
	this.mPositionCenter.x += myRandom() * window.innerWidth;
	this.mPositionCenter.y += myRandom() * window.innerHeight;
	...
	this.mSpeed.x = myRandom() * window.innerWidth * 0.2;
	this.mSpeed.y = myRandom() * window.innerWidth * 0.2;
	this.mSpeedTimer = 0.;
	...
	{
		if(this.mParent.playState == 0)
		{
			InitAudio(this.mParent.url);
			this.mParent.playState = 1;
		}
	...
	particle.targetOpacity = function(opacity)
	{
		this.material.opacity += (opacity - this.material.opacity) * 0.03;
		this.TargetObject.info.material.opacity = this.material.opacity;
	}
...
ParticleSound.prototype.Update = function(delta)
{
	if(this.playState > 0)
	{
		this.mParticle.targetOpacity(1.);
		return;	
	}
	...
	if(sPlayingSound && (this.mParticle.position.distanceTo(cameraManager.GetPosition()) < ParticleGroups[sGroupCurrent].cameraDistance * 1.2))
	{
		this.mParticle.targetOpacity(0.01);
	}
	else
	{
		this.mParticle.targetOpacity(1.);
