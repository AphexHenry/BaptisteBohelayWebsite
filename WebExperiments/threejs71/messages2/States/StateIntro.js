...
function BehaviourIntro(aPart, aPosition) 
{
	...
	var lPartCount = sParticlesManager.GetCount();
	var lNumPartX = Math.floor(Math.sqrt(lPartCount * getRatio()));
	var lNumPartY = Math.floor(lPartCount / lNumPartX);
	...
	this.mAttractorMatrix = new THREE.Vector3(lXposition, lYposition, aPosition.z);//aPosition.clone().multiplyScalar(2.);
...
//	part->mVelocity.z += (-1500 - part->mPosition.z) * aTimeInterval;
	...
	this.part.mVelocity.add(MinusMult(lAttractorMatrix, this.part.mPosition, 15.5 * aTimeInterval / lDistance));
...
{	
    ...
    lDistance = Math.max(attractorPosition.distanceTo(this.part.mPosition), 0.4) / 2.;

 	this.part.mVelocity.add(new THREE.Vector3().subVectors(lAttractorMatrix, this.part.mPosition).multiplyScalar( lStrengthSpring * aTimeInterval));
 	this.part.mSizeSpeed += 50 * (ATTRACTOR_IDLE_SIZE * Math.min(lDistance * 3., 1.) - this.part.mSize) * aTimeInterval;
 	this.part.mSizeSpeed *= 0.95;

	 // compute strength from attractor.
    this.part.mVelocity.add(MinusMult(attractorPosition, this.part.mPosition, 1. * lStrength * Attractors[0].strengthCoeff * aTimeInterval / (lDistance * lDistance)));		

	var rotTarget = new THREE.Vector3(-(1 / yRel ) * rotSpeed, (1 / xRel ) * rotSpeed, 0/*(Math.random() - 0.5) * rotSpeed*/);

	var ampRot = 1.;
	var xRel = THREE.Math.clamp(ampRot * (lAttractorMatrix.x - this.part.mPosition.x), -1., 1);
	var yRel = THREE.Math.clamp(ampRot * (lAttractorMatrix.y - this.part.mPosition.y), -1, 1);
	var zRel = THREE.Math.clamp(ampRot * (lAttractorMatrix.z - this.part.mPosition.z), -1, 1);
	var rotSpeed = Math.PI / 2;
	this.part.mRotationSpeed = new THREE.Vector3(0, 0, 0);
	this.part.mRotation.x = -yRel  * rotSpeed / 5;
	this.part.mRotation.y = -xRel * rotSpeed + zRel * rotSpeed;
