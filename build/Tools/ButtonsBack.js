...
ButtonsBack.prototype.OnChange = function()
{
	...
	{
		// iCurr = sGroupCurrent;
		listBack.push({index:sGroupCurrent ,color:new THREE.Color().setRGB(0.8,0.7,0.7), name:sTools.ParticleGroups[sGroupCurrent].name});
	}
	else
	{
		iCurr = Organigram.GetFather(sGroupCurrent);	
		while(iPrev >= 0 && (sGroupCurrent != sTools.ParticleGroup.PART_CREA_LULU) && (sGroupCurrent != sTools.ParticleGroup.PART_INTRO))
		{
			iPrev = Organigram.GetFather(iCurr);
			if(iPrev < 0)
			{
				listBack.push({index:sTools.ParticleGroup.PART_CREA_LULU ,color:new THREE.Color().setRGB(0.8,0.7,0.7), name:"home"});
				break;
			}

			var particle = sTools.ParticleGroups[iPrev].GetParticleThatLeadTo(iCurr);
